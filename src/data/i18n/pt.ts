export const pt = {
  header: ["Início", "Sobre", "Catálogo", "Contato", "Whatsapp"],
  hero: {
    title1: {
      start: "Terços artesanais feitos com ",
      highlight1: "fé ",
      mid: "e ",
      highlight2: "carinho",
      end: ".",
    },
    description:
      "Terços artesanais, duráveis e belos, feitos com fé e carinho para honrar a Jesus e a Santíssima Virgem.",
    button1: "Ver mais",
    button2: "Contato",
    verse: {
      text1: "Rezem o terço todos os dias...",
      text2: "-Virgem de Fátima",
    },
  },
  about: {
    title: "Sobre",
    texts: [
      {
        title: {
          start: "Olá, eu sou ",
          highlight1: "Ana Elisa",
          mid: "",
          highlight2: "",
          end: ". [ 1 ]",
        },
        text: "Sempre acreditei que a fé se vive nas pequenas coisas, nos gestos simples e na forma como escolhemos enxergar a vida. É em Deus que encontro calma, direção e força, e é essa confiança que guia quem eu sou e tudo o que faço.",
      },
      {
        title: {
          start: "Cada ",
          highlight1: "Terço ",
          mid: "que produzo é feito em ",
          highlight2: "oração",
          end: ". [ 2 ]",
        },
        text: "Para mim, ele não é apenas um objeto, mas um símbolo de devoção, um instrumento de encontro com Deus e uma forma de espalhar a paz que sinto enquanto crio. Em cada detalhe coloco carinho, intenção e fé, para que quem receba também sinta essa presença.",
      },
      {
        title: {
          start: "A ",
          highlight1: "Divinum Rosarium ",
          mid: "nasceu com uma ",
          highlight2: "missão",
          end: ". [ 3 ]",
        },
        text: "Mais do que uma loja, é uma forma de unir trabalho e propósito: levar adiante a fé, a esperança e o amor que me acompanham em cada terço. Meu desejo é que cada pessoa que o receba se sinta acolhida, fortalecida e mais próxima de Deus.",
      },
    ],
  },
  catalog: {
    title: "CATÁLOGO",
    attributes: ["ARTESANAL", "DELICADO", "SÚTIL", "GRACIOSO"],
    verse: {
      text1: "Orai sem cessar...",
      text2: "-1 Tessalonicenses 5:17",
    },
  },
  product: {
    title1: {
      start: "Cada ",
      highlight1: "terço ",
      mid: "é ",
      highlight2: "único",
      end: ".",
    },
    title2: {
      start: "Escolha com ",
      highlight: "carinho ",
      end: "os detalhes do seu.",
    },
    description:
      "A personalização é parte da oração. Cada escolha carrega uma intenção.",
    button: "Encomendar",
    options: [
      {
        name: "Pedra de dezena",
        description: "Cor que mais representa sua intenção.",
        choices: ["Branco", "Azul", "Vinho", "Roxo"],
      },
      {
        name: "Pedra dos mistérios",
        description: "Cor que mais representa os momentos da oração.",
        choices: ["Branco", "Azul", "Vinho", "Roxo"],
      },
      {
        name: "Entremeio",
        description: "Medalha que acompanha seu caminho espiritual.",
        choices: ["Metal", "Madeira", "Resina", "Emborrachado"],
      },
      {
        name: "Crucifixo",
        description: "Símbolo central da fé.",
        choices: ["Metal", "Madeira", "Resina", "Emborrachado"],
      },
      {
        name: "Fio",
        description: "Fio que sustenta e mantém tudo unido",
        choices: ["Encerado", "Torcido", "Seda", "Algodão"],
      },
    ],
  },
  contact: {
    infos: ["Email", "Instagram", "Whatsapp"],
    title1: "Contato",
    title2: {
      start: "Entre em ",
      highlight: "contato",
    },
    description:
      "Entre em contato para tirar suas dúvidas ou conversar diretamente comigo sobre os terços. Será um prazer falar com você!",
    form: {
      inputs: ["Nome *", "Email *", "Seu assunto *", "Sua mensagem *"],
      button: "Enviar mensagem",
    },
  },
};
